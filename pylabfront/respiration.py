    ...
    for participant in participant_id:
        try:
            ...
            if len(respiratory_data) > 0:
                data_dict[participant] = respiratory_data.groupby(respiratory_data[loader.date_column].dt.date)[
                    loader.respiratorion_column].mean().to_dict()
        except:
...
def get_average_breaths_per_minute(loader, day_or_night=None, start_date=None, end_date=None, participant_id="all"):
    ...
    """
    data_dict = get_breaths_per_minute_per_day(
        loader, day_or_night, start_date, end_date, participant_id)
    average_dict = {}
    for participant in data_dict.keys():
        average_dict[participant] = np.array(
            list(data_dict[participant].values())).nanmean()
    return average_dict
